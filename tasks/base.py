        ...
        for example_id in range(len(inputs)):
            ...
            for style_id in range(len(inputs[example_id])):
                ...
                with context_manager:
                    ...
                    h = forward_trace.residual_stream.hidden # among the layers
                    # print(len(h)) # 33 layers
                # print(h[0].size()) # [1, n_token, dim]
                embedding_token = []
                for layer in range(len(h)):
                    embedding_token.append(h[layer][:,-1]) # taking the last token of x_i or y_i
                embedding_token = torch.cat(embedding_token, dim=0).cpu().clone() # [num_layers, dim] # last token of x_i or y_i
                embeddings_for_all_styles.append(embedding_token) # [2, numlayers*dim] # last token of x_i or y_i
            h_all.append(tuple(embeddings_for_all_styles)) # [n_example, 2, numlayers*dim]
        return h_all
