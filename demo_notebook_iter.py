...
encoding_model = SentenceTransformer("math-similarity/Bert-MLM_arXiv-MP-class_zbMath")
...
def sampling_icv(engine, steps):
    ...
    prompts = [(text,
                SamplingParams(temperature=0.65,
                               max_tokens=600,
                               stop=["###"]),
                ControlVectorRequest("chaotic", 1, cv_path, scale=0.11))]
...
def multisampling_icv(engine, steps:List[str], nsteps: int, idx: int) -> Union[str, torch.Tensor]:
    text = "".join(steps)
    text += f"### Step {len(steps)}: "
    prompts = [(text,
                SamplingParams(temperature=0.65,
...
# Function to compute diversity within a list of embeddings
def compute_diversity(embeddings):
    similarity_matrix = cosine_similarity(embeddings)
    np.fill_diagonal(similarity_matrix, 0)  # Ignore self-similarity
    avg_similarity = np.mean(similarity_matrix)
    diversity_score = 1 - avg_similarity  # Diversity is the inverse of similarity
    return diversity_score

def compute_difference(embeddings_1, embeddings_2):
